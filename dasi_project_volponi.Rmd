...
Most of the soccer games of this championship were video recorded and analysed. It is almost a 'census' of that soccer championship, covering 98% of the matches and 100% of the players. The data avaliable if for 217 of 220 already played matches of 2014 (22 rounds passed, 38 rounds in total).
...
The data avaliable if for the first 22 rounds of the 38 total rounds (as the championship is ongoing), and 217 of total 380 games were analised (57.1%). With this amount of data, we are pretty sure that is possible to sample with enough confidence to generalize results for the whole championship.
...
* Data from the beginning of the year - we can't be sure if the "home effect" vary in the beggning of the end of the championship. The data available is just for the 57% first matches.
...
To begin our analysis, we will plot the histogram and the boxplot of the data.
We can say that the player score forms a nearly normal distribution, somewhat simmetrical and slighly right-skewed.
...
This information sugests that there is a 
...
## rascunhos ##
The data represents performances (scores based on scouts) of all players in 217 of the first 220 matches (98.63%) at the Brazilian soccer championship.

For each relevant action done by the player (for example, a successful pass or a goal missed) there is a score (ranging from -6 to a own goal up to +8 point to scoring a goal). All the actions the player does is tabulated and scored. The resulting number (player match score) indicates the overall influence the player had in a match (negative, neutral or positive).
